...
func (p *Provider) Name() string { return "wazirx" }
...
func (p *Provider) GetPrices(ctx context.Context, coinIDs []string, currency string) ([]model.Price, error) {
	...
	for _, coinID := range coinIDs {
		...
		pair := sym + quote
		url := fmt.Sprintf("%s/sapi/v1/ticker/24hr?symbol=%s", p.cfg.BaseURL, pair)
		...
		var raw wazirxTicker
		if err := json.NewDecoder(resp.Body).Decode(&raw); err != nil {
			p.recordError(err)
			return nil, fmt.Errorf("decoding response: %w", err)
		}

		val, err := decimal.NewFromString(raw.LastPrice)
