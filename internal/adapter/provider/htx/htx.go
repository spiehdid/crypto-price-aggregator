...
func (p *Provider) Name() string { return "htx" }
...
func (p *Provider) GetPrices(ctx context.Context, coinIDs []string, currency string) ([]model.Price, error) {
	...
	for _, coinID := range coinIDs {
		...
		symbol := sym + quote
		url := fmt.Sprintf("%s/market/detail/merged?symbol=%s", p.cfg.BaseURL, symbol)
		...
		if raw.Status != "ok" || raw.Tick.Close == 0 {
			continue
		}
		...
		pr := model.Price{
			CoinID:     coinID,
			Currency:   currency,
			Value:      decimal.NewFromFloat(raw.Tick.Close),
			Provider:   "htx",
			Timestamp:  now,
			ReceivedAt: now,
		}
		if raw.Tick.Vol > 0 {
			pr.Volume24h = decimal.NewFromFloat(raw.Tick.Vol)
		}
