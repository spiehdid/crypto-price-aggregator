...
func TestGetPrice_Success(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		assert.Equal(t, "/v1/getData", r.URL.Path)
		assert.Equal(t, "BTC", r.URL.Query().Get("symbol"))

		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"status":200,"data":{"BTC":{"price":67432.15,"volume":25000000000,"marketCap":1320000000000,"percentChange24h":2.5}}}`))
	}))
	defer server.Close()

	provider := freecryptoapi.New(freecryptoapi.Config{
		BaseURL:   server.URL,
		RateLimit: 30,
	})
	...
	assert.Equal(t, "freecryptoapi", price.Provider)
...
func TestGetPrice_RateLimited(t *testing.T) {
	...
	provider := freecryptoapi.New(freecryptoapi.Config{BaseURL: server.URL})
...
func TestGetPrice_UnknownCoin(t *testing.T) {
	provider := freecryptoapi.New(freecryptoapi.Config{BaseURL: "http://localhost"})
...
func TestName(t *testing.T) {
	provider := freecryptoapi.New(freecryptoapi.Config{BaseURL: "http://localhost"})
	assert.Equal(t, "freecryptoapi", provider.Name())
}
