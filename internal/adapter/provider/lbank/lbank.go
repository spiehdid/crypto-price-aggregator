...
func (p *Provider) Name() string { return "lbank" }
...
type lbankEntry struct {
	Symbol    string      `json:"symbol"`
	Ticker    lbankTicker `json:"ticker"`
	Timestamp int64       `json:"timestamp"`
}

type lbankResponse struct {
	Result string       `json:"result"`
	Data   []lbankEntry `json:"data"`
}
...
func (p *Provider) GetPrices(ctx context.Context, coinIDs []string, currency string) ([]model.Price, error) {
	...
	for _, coinID := range coinIDs {
		...
		pair := sym + "_" + quote
		url := fmt.Sprintf("%s/v2/ticker.do?symbol=%s", p.cfg.BaseURL, pair)
		...
		entry := raw.Data[0]
		val, err := decimal.NewFromString(entry.Ticker.Latest)
		...
		if vol, err := decimal.NewFromString(entry.Ticker.Vol); err == nil {
			pr.Volume24h = vol
		}
