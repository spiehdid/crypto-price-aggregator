...
func (p *Provider) Name() string { return "phemex" }
...
type phemexTickerResult struct {
...
type phemexResponse struct {
	Error  interface{}        `json:"error"`
	Result phemexTickerResult `json:"result"`
}
...
func (p *Provider) GetPrices(ctx context.Context, coinIDs []string, currency string) ([]model.Price, error) {
	...
	for _, coinID := range coinIDs {
		...
		symbol := "s" + sym + "USDT"
		url := fmt.Sprintf("%s/md/v1/ticker/24hr?symbol=%s", p.cfg.BaseURL, symbol)
		...
		var raw phemexResponse
		if err := json.NewDecoder(resp.Body).Decode(&raw); err != nil {
			p.recordError(err)
			return nil, fmt.Errorf("decoding response: %w", err)
		}

		if raw.Result.LastEp == 0 {
			continue
		}

		val := decimal.NewFromInt(raw.Result.LastEp).Div(decimal.NewFromFloat(scaleFactor))
