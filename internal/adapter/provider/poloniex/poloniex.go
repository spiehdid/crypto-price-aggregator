...
func (p *Provider) Name() string { return "poloniex" }
...
func (p *Provider) GetPrices(ctx context.Context, coinIDs []string, currency string) ([]model.Price, error) {
	...
	for _, coinID := range coinIDs {
		...
		pair := sym + "_" + quote
		url := fmt.Sprintf("%s/markets/%s/price", p.cfg.BaseURL, pair)
		...
		if raw.DailyChange != "" {
			if change, err := decimal.NewFromString(raw.DailyChange); err == nil {
				f, _ := change.Mul(decimal.NewFromInt(100)).Float64()
				price.Change24h = f
			}
		}
