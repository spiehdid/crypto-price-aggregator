...
func TestGetPrice_Success(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		assert.Equal(t, "/markets/BTC_USDT/price", r.URL.Path)

		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"symbol":"BTC_USDT","price":"67432.15","dailyChange":"0.0119","time":1648866396349}`))
	}))
	defer server.Close()

	provider := poloniex.New(poloniex.Config{BaseURL: server.URL, RateLimit: 20})
	...
	assert.Equal(t, "poloniex", price.Provider)
	assert.InDelta(t, 1.19, price.Change24h, 0.001)
}
...
func TestGetPrice_RateLimited(t *testing.T) {
	...
	provider := poloniex.New(poloniex.Config{BaseURL: server.URL})
...
func TestName(t *testing.T) {
	provider := poloniex.New(poloniex.Config{BaseURL: "http://localhost"})
	assert.Equal(t, "poloniex", provider.Name())
}
