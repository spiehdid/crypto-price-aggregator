...
func (p *Provider) Name() string { return "btse" }
...
func (p *Provider) GetPrices(ctx context.Context, coinIDs []string, currency string) ([]model.Price, error) {
	...
	for _, coinID := range coinIDs {
		...
		pair := sym + "-" + quote
		url := fmt.Sprintf("%s/spot/api/v3.2/market_summary?symbol=%s", p.cfg.BaseURL, pair)
		...
		entry := raw[0]
		val := decimal.NewFromFloat(entry.Last)
		if val.IsZero() {
			continue
		}
		...
		prices = append(prices, model.Price{
			...
			Timestamp:  now,
			ReceivedAt: now,
			Change24h:  entry.PercentageChange,
			Volume24h:  decimal.NewFromFloat(entry.Volume),
		})
	}
