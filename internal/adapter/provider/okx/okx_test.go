...
func TestGetPrice_Success(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"code":"0","data":[{"instId":"BTC-USDT","last":"67432.15","vol24h":"5678","volCcy24h":"245000000"}]}`))
	}))
	defer server.Close()

	provider := okx.New(okx.Config{BaseURL: server.URL, RateLimit: 20})
	...
	assert.Equal(t, "okx", price.Provider)
...
func TestGetPrice_RateLimited(t *testing.T) {
	...
	provider := okx.New(okx.Config{BaseURL: server.URL})
...
func TestName(t *testing.T) {
	provider := okx.New(okx.Config{BaseURL: "http://localhost"})
