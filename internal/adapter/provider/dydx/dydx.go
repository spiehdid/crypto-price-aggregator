...
func (p *Provider) Name() string { return "dydx" }
...
func (p *Provider) GetPrices(ctx context.Context, coinIDs []string, currency string) ([]model.Price, error) {
	...
	for _, coinID := range coinIDs {
		...
		ticker := sym + "-USD"
		url := fmt.Sprintf("%s/v4/perpetualMarkets?ticker=%s", p.cfg.BaseURL, ticker)
		...
		var raw dydxResponse
		if err := json.NewDecoder(resp.Body).Decode(&raw); err != nil {
			p.recordError(err)
			return nil, fmt.Errorf("decoding response: %w", err)
		}

		market, ok := raw.Markets[ticker]
		if !ok || market.OraclePrice == "" {
			continue
		}

		val, err := decimal.NewFromString(market.OraclePrice)
		if err != nil {
			continue
		}
		...
		if vol, err := decimal.NewFromString(market.Volume24H); err == nil {
			pr.Volume24h = vol
		}
