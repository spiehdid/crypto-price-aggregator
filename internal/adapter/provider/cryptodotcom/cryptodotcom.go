...
func (p *Provider) Name() string { return "cryptodotcom" }
...
type cryptodotcomTickerEntry struct {
	Instrument string `json:"i"`
	Last       string `json:"a"`
	Volume     string `json:"v"`
}

type cryptodotcomResult struct {
	Data []cryptodotcomTickerEntry `json:"data"`
}

type cryptodotcomResponse struct {
	Code   int                `json:"code"`
	Result cryptodotcomResult `json:"result"`
}
...
func (p *Provider) GetPrices(ctx context.Context, coinIDs []string, currency string) ([]model.Price, error) {
	...
	for _, coinID := range coinIDs {
		...
		pair := sym + "_" + quote
		url := fmt.Sprintf("%s/exchange/v1/public/get-tickers?instrument_name=%s", p.cfg.BaseURL, pair)
		...
		entry := raw.Result.Data[0]
		val, err := decimal.NewFromString(entry.Last)
		...
		price := model.Price{
			...
			Currency:   currency,
			Value:      val,
			Provider:   "cryptodotcom",
			Timestamp:  now,
			ReceivedAt: now,
		}

		if entry.Volume != "" {
			if vol, err := decimal.NewFromString(entry.Volume); err == nil {
				price.Volume24h = vol
			}
		}
