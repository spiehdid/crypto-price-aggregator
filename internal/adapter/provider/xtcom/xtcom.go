...
func (p *Provider) Name() string { return "xtcom" }
...
type xtcomTickerEntry struct {
	Symbol string `json:"s"`
	Time   int64  `json:"t"`
	Price  string `json:"p"`
}

type xtcomResponse struct {
	RC     int                `json:"rc"`
	MC     string             `json:"mc"`
	Result []xtcomTickerEntry `json:"result"`
}
...
func (p *Provider) GetPrices(ctx context.Context, coinIDs []string, currency string) ([]model.Price, error) {
	...
	for _, coinID := range coinIDs {
		...
		pair := sym + "_" + quote
		url := fmt.Sprintf("%s/v4/public/ticker/price?symbol=%s", p.cfg.BaseURL, pair)
		...
		val, err := decimal.NewFromString(raw.Result[0].Price)
