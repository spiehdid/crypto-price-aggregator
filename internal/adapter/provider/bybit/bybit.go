...
func (p *Provider) GetPrices(ctx context.Context, coinIDs []string, currency string) ([]model.Price, error) {
	...
	for _, coinID := range coinIDs {
		...
		symbol := sym + quote
		url := fmt.Sprintf("%s/v5/market/tickers?category=spot&symbol=%s", p.cfg.BaseURL, symbol)
		...
		ticker := raw.Result.List[0]
		val, err := decimal.NewFromString(ticker.LastPrice)
		if err != nil {
			continue
		}
		pr := model.Price{
		...
		if vol, err := decimal.NewFromString(ticker.Turnover24h); err == nil {
			pr.Volume24h = vol
		}
		if pct, err := strconv.ParseFloat(ticker.Price24hPcnt, 64); err == nil {
			pr.Change24h = pct * 100
		}
