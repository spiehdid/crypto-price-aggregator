...
func TestGetPrice_Success(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"last":"67432.15","high":"68000","low":"66000","vwap":"67200","volume":"5678.90","bid":"67430","ask":"67435","timestamp":"1700000000","open":"66500"}`))
	}))
	defer server.Close()

	provider := bitstamp.New(bitstamp.Config{BaseURL: server.URL, RateLimit: 20})
	...
	assert.Equal(t, "bitstamp", price.Provider)
...
func TestGetPrice_RateLimited(t *testing.T) {
	...
	provider := bitstamp.New(bitstamp.Config{BaseURL: server.URL})
...
func TestName(t *testing.T) {
	provider := bitstamp.New(bitstamp.Config{BaseURL: "http://localhost"})
	assert.Equal(t, "bitstamp", provider.Name())
}

func TestGetPrice_CoinNotFound(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"last":"","high":"","low":"","vwap":"","volume":"","bid":"","ask":"","timestamp":"","open":""}`))
	}))
	defer server.Close()
